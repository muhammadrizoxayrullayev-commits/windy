// Russian Wordlist and Quotes

export const russianWords = [
  "и", "в", "не", "на", "я", "быть", "он", "с", "что", "а",
  "по", "это", "она", "этот", "к", "но", "они", "мы", "как", "из",
  "у", "который", "то", "за", "свой", "что", "весь", "год", "от", "так",
  "о", "для", "ты", "же", "все", "тот", "мочь", "вы", "человек", "такой",
  "его", "сказать", "только", "или", "еще", "бы", "себя", "один", "как", "уже",
  "до", "время", "если", "сам", "когда", "другой", "вот", "говорить", "наш", "мой",
  "знать", "стать", "при", "чтобы", "дело", "жизнь", "кто", "первый", "очень", "два",
  "день", "рука", "даже", "во", "со", "раз", "где", "там", "под", "можно",
  "ветер", "небо", "скорость", "мысль", "свет", "свобода", "тишина", "река", "море", "дождь",
  "солнце", "мир", "путь", "шаг", "сердце", "душа", "мечта", "слово", "звук", "книга"
];

export const russianQuotes = [
  {
    text: "Красота спасет мир, если мир спасет красоту.",
    source: "Фёдор Достоевский"
  },
  {
    text: "Учитесь так, словно вы постоянно ощущаете нехватку своих знаний.",
    source: "Мудрость"
  },
  {
    text: "Ветер не может сломать дерево, которое умеет гнуться.",
    source: "Пословица"
  }
];
